/*
   weizz - map display utility
   ---------------------------

   Written and maintained by Andrea Fioraldi <andreafioraldi@gmail.com>

   Based on afl-showmap by Michal Zalewski <lcamtuf@google.com>
...
static u32 write_results(void) {
  ...
  u8 cco = !!getenv("WEIZZ_CMIN_CRASHES_ONLY"),
     caa = !!getenv("WEIZZ_CMIN_ALLOW_ANY");
...
static void show_banner(void) {
  ...
  SAYF(cCYA "weizz-showmap " cBRI VERSION cRST
...
static char **get_qemu_argv(u8 *own_loc, char **argv, int argc) {
  ...
  if (tmp) {
    ...
    cp = alloc_printf("%s/weizz-qemu", tmp);
  ...
  if (rsl) {
    ...
    *rsl = 0;
    ...
    cp = alloc_printf("%s/weizz-qemu", own_copy);
    ck_free(own_copy);
  ...
  FATAL("Unable to find 'weizz-qemu'.");
...
int main(int argc, char **argv) {
  ...
  if (getenv("WEIZZ_PATCH_MAP") == NULL) setenv("WEIZZ_DISABLE_PATCH", "1", 1);
