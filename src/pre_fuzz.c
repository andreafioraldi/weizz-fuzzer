/*
   weizz - routines for fuzz_one() preparation
   -------------------------------------------
...
static void write_with_gap(void *mem, u32 len, u32 skip_at, u32 skip_len) {
  ...
  if (out_file) {
    ...
    fd = locked_open_mode(out_file, O_WRONLY | O_CREAT | O_EXCL, 0600);
...
u8 calibrate_case(struct queue_entry *q, u8 *use_mem, u32 handicap,
  ...
  if (!light_forksrv_pid) { init_light_forkserver(light_argv); }
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    fault = run_light_target(use_tmout);
...
u8 trim_case(struct queue_entry *q, u8 *in_buf) {
  ...
  if (needs_write) {
    ...
    unlink(q->fname);                       /* ignore errors, possible race */

    fd = locked_open_mode(q->fname, O_WRONLY | O_CREAT | O_EXCL, 0600);
