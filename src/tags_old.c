...
/* the chunk type i sthe cmpid of the first tag.
   the build_kind is the method used to reconstruct the chunk */

#define TAG_EQUALS(a, b) ((a).cmp_id == (b).cmp_id)

static u32 adjust_tags_counters(tag_t *tags, s32 i, u32 len, u16 new_cnt) {
...
static u32 count_ntypes(tag_t *tags, s32 i, u32 len) {
...
static u32 search_field_end(tag_t *tags, tag_t *last, u32 i, u32 len,
                            u32 initial) {
...
static void field_mutator(u8 *out_buf, s32 begin, s32 end) {
  ...
  switch (UR(12 + ((extras_cnt + a_extras_cnt) ? 1 : 0))) {
    ...
    case 1: {
      ...
      out_buf[(UR(end - begin) + begin)] = val;
    ...
    case 5: {
      ...
      out_buf[(UR(end - begin) + begin)] -= 1 + UR(ARITH_MAX);
    ...
    case 11: {
      ...
      out_buf[(UR(end - begin) + begin)] ^= 1 + UR(255);
...
static void search_field_boundaries(tag_t *tags, s32 i, u32 len, s32 *start,
                                    s32 *end) {
...
static int get_random_field(tags_info_t *tags, s32 len, s32* start, s32* end, u8 for_mut) {
  ...
  if (UR(2)) {
    ...
    for (i = UR(len); i < len;) {
      ...
      u16 cmp_id = tags->tags[i].cmp_id;

      search_field_boundaries(tags->tags, i, len, &i, &j);

      /* 1/3 probability to mutate input-to-states */
      if (!for_mut || !(tags->tags[i].flags & TAG_IS_INPUT_TO_STATE) ||
          UR(3) == 0) {
    ...
    for (i = 0; i < len; ++i) {
      ...
      if (k == idx) {
        ...
        j = search_field_end(tags->tags, &tags->tags[i], i, len, i);
...
bail_out_field:
      if (last_tg != tags->tags[i].cmp_id) ++k;
      last_tg = tags->tags[i].cmp_id;
...
static void mutate_random_field(tags_info_t *tags, u8 *out_buf, s32 len) {

  s32 i, j;
  if (get_random_field(tags, len, &i, &j, 1))
    return;
...
static int field_of_type(tags_info_t *tags, u8 *out_buf, s32 len, u16 type,
                         s32* start, s32* end) {
  ...
  for (i = UR(len); i < len;) {
    ...
    search_field_boundaries(tags->tags, i, len, start, end);
    return 1;
...
static u32 search_chunk_end(tag_t *tags, tag_t *last, s32 i, u32 len) {
...
/*
static u32 reverse_search_chunk_end(tag_t *tags, tag_t *tgt, s32 i, u32 len) {
...
static void search_chunk_boundaries(tag_t *tags, s32 i, u32 len, s32 *start,
                                    s32 *end) {
...
static u32 search_next_its(tag_t *tags, tag_t *last, u32 i, u32 len) {
...
static u32 search_next_rnd(tag_t *tags, tag_t *last, u32 i, u32 len) {
...
static u8 get_chunk_of_type(tag_t *tags, u32 len, u16 type, u32 target_len, u8 build_kind, u32 *start, u32*end) {
...
static u8 get_chunk_of_parent(tag_t *tags, u32 len, u16 parent, u8 build_kind, u32 *start, u32*end) {
...
/*
static struct worklist *get_chunks_between_its(tag_t *tags, u32 len,
                                               s32 *number) {
...
static int get_field_from_type_index(tag_t *tags, u32 len, u32 idx, s32* start, s32* end) {
...
static int get_rnd_chunk_from_type_index(tag_t *tags, u32 len, u32 idx, s32* start, s32* end) {
...
static int get_chunk_from_type_index(tag_t *tags, u32 len, u32 idx, s32* start, s32* end) {
...
static u8 get_random_chunk(tags_info_t *tags, s32 temp_len, s32 *chunk_start,
...
u8 higher_order_fuzzing(tags_info_t **tags, s32 *temp_len, u8 **out_buf,
                        s32 alloc_size) {
  ...
  switch (s) {
    ...
    case 0: {                                               /* Delete chunk */
      ...
      if (get_random_chunk(*tags, *temp_len, &del_from, &del_to, build_kind))
          break;

      // avoid to have an empty out_buf
      if (del_to - del_from == (*temp_len)) break;

      // DBGPRINT(" TAGS DELETE %d %d\n", del_from, del_to);
      
      (*tags)->ntypes -= count_ntypes((*tags)->tags, del_from, del_to);

      memmove((*out_buf) + del_from, (*out_buf) + del_to, (*temp_len) - del_to);
      memmove((*tags)->tags + del_from, (*tags)->tags + del_to,
              ((*temp_len) - del_to) * sizeof(tag_t));
    ...
    case 1: {                                               /* Splice chunk */
      ...
      if (get_random_chunk(*tags, *temp_len, &target_start_byte,
                             &target_end_byte, build_kind))
        break;

      type = (*tags)->tags[target_start_byte].cmp_id;
      ...
      tags_info_t *source_tags = ck_alloc_nozero(
          sizeof(tags_info_t) + source_entry->len * sizeof(tag_t));
      ...
      if (t_stat.st_size != sizeof(tags_info_t) + sizeof(tag_t) * source_entry->len) {
      
        ck_free(source_tags);
        //ck_free(target->tags_fname);
        source_entry->tags_fname = NULL;
        source_entry->passed_getdeps = 0;
        source_entry->cached_tags_ntypes = 0;
        source_entry->tags_coverage = 0;
        source_entry->use_derived_tags    = 0;
      ...
      u32 tgt_ntypes = count_ntypes((*tags)->tags, target_start_byte, target_start_byte + source_len);

      memcpy((*out_buf) + target_start_byte, source_buf + source_start_byte,
             source_len);
      ...
      u32 src_ntypes = adjust_tags_counters(source_tags->tags, source_start_byte, source_end_byte, (*tags)->tags[target_start_byte].counter);
      
      (*tags)->ntypes -= tgt_ntypes;
      (*tags)->ntypes += src_ntypes;

      memcpy((*tags)->tags + target_start_byte,
             source_tags->tags + source_start_byte,
             source_len * sizeof(tag_t));

      memmove(
          (*tags)->tags + target_start_byte + source_len,
          (*tags)->tags + target_start_byte + target_len,
          ((*temp_len) - target_start_byte - target_len) * sizeof(tag_t));
    ...
    case 2: {
      ...
      parent = (*tags)->tags[target_start_byte].parent;
      ...
      if (source_len > 0) {
        ...
        tags_info_t *new_tags = ck_alloc_nozero(
            sizeof(tags_info_t) + ((*temp_len) + source_len) * sizeof(tag_t));
        ...
        memcpy(new_buf, *out_buf, split_idx);
        memcpy(new_buf + split_idx, source_buf + source_start_byte,
               source_len);
        memcpy(new_buf + split_idx + source_len, (*out_buf) + split_idx,
               (*temp_len) - split_idx);

        u32 src_ntypes = adjust_tags_counters(source_tags->tags, source_start_byte, source_end_byte, (*tags)->tags[target_start_byte].counter);
        
        new_tags->ntypes = (*tags)->ntypes + src_ntypes;

        memcpy(new_tags->tags, (*tags)->tags, split_idx * sizeof(tag_t));
        memcpy(new_tags->tags + split_idx,
               source_tags->tags + source_start_byte,
               source_len * sizeof(tag_t));
        memcpy(new_tags->tags + split_idx + source_len,
               (*tags)->tags + split_idx,
               (*temp_len - split_idx) * sizeof(tag_t));
    ...
    case 3 ... 5: {
      ...
      mutate_random_field(*tags, *out_buf, *temp_len);
      break;
