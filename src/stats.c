...
void show_stats(void) {
  ...
  sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
          crash_mode ? cPIN "peruvian were-rabbit" : cYEL "weizz fuzzer",
          use_banner);
...
#define SP20 SP10 SP10
  ...
  if ((last_path_time || resuming_fuzz || queue_cycle == 1 || in_bitmap ||
       crash_mode)) {
  ...
  SAYF(bV bSTOP "    last unpatch : " cRST "%-34s " bSTG bV bSTOP
                "      taggeds : " cRST "%-6s " bSTG        bV "\n",
       DTD(cur_ms, last_ckunpatch_time), DI(tg_queued_num));
  ...
  SAYF("    map density : %s%-21s " bSTG bV "\n",
       t_byte_ratio > 70 ? cLRD : ((t_bytes < 200) ? cPIN : cRST), tmp);
  ...
  SAYF(" stability : %s%-10s " bSTG bV "\n",
       (stab_ratio < 85 && var_byte_count > 40)
           ? cLRD
           : ((queued_variable && (var_byte_count > 20)) ? cMGN : cRST),
       tmp);

  /* weizz */

  sprintf(tmp, "%s/%s, %s/%s", DI(stage_finds[STAGE_TAGS_HAVOC]),
          DI(stage_cycles[STAGE_TAGS_HAVOC]),
          DI(stage_finds[STAGE_TAGS_SPLICE]),
          DI(stage_cycles[STAGE_TAGS_SPLICE]));

  SAYF(bV bSTOP "  tags havoc : " cRST "%-37s " bSTG bV bSTOP, tmp);

  sprintf(tmp, "%s (%s)", DI(patched_cksums_num), DI(patched_cksums_total_num));

  SAYF(" checksums : " cRST "%-10s " bSTG bV "\n", tmp);

  sprintf(tmp, "%s/%s %s/%s", DI(stage_finds[STAGE_COLORIZATION]),
          DI(stage_cycles[STAGE_COLORIZATION]), DI(stage_finds[STAGE_GETDEPS]),
          DI(stage_cycles[STAGE_GETDEPS]));

  SAYF(bV bSTOP "     getdeps : " cRST "%-37s " bSTG bV bSTOP, tmp);

  sprintf(tmp, "%s/%s", DI(getdeps_fix), DI(getdeps_fix_total));

  SAYF("   Q fixes : " cRST "%-10s " bSTG bV "\n", tmp);

  sprintf(tmp, "%s/%s %s/%s", DI(stage_finds[STAGE_ITS]),
          DI(stage_cycles[STAGE_ITS]), DI(stage_finds[STAGE_LOCKED_HAVOC]),
          DI(stage_cycles[STAGE_LOCKED_HAVOC]));

  SAYF(bV bSTOP "    surgical : " cRST "%-37s " bSTG bV bSTOP, tmp);

  sprintf(tmp, "%s/%s", DI(crash_fix), DI(crash_fix_total));

  SAYF("   C fixes : " cRST "%-10s " bSTG bV "\n", tmp);

  //sprintf(tmp, "%0.02f%%", max_tags_coverage);

  //SAYF("  tags cov : " cRST "%-10s " bSTG bV "\n", tmp);
...
void show_init_stats(void) {
  ...
  if (avg_us > 50000)
    WARNF(cLRD "The target binary is pretty slow! See AFL/doc/perf_tips.txt.");
  ...
  if (!resuming_fuzz) {
    ...
    if (max_len > 50 * 1024)
      WARNF(cLRD "Some test cases are huge (%s) - see AFL/doc/perf_tips.txt!",
            DMS(max_len));
    else if (max_len > 10 * 1024)
      WARNF("Some test cases are big (%s) - see AFL/doc/perf_tips.txt.",
            DMS(max_len));
    ...
    if (queued_paths > 100)
      WARNF(cLRD
            "You probably have far too many input files! Consider "
            "trimming down.");
...
void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  fprintf(f,
          ...
          crash_mode ? "crash " : "", deferred_mode ? "deferred " : "",
          (crash_mode || deferred_mode) ? "" : "default", orig_cmdline);
  /* ignore errors */
