...
static void remove_shm(void) {
  ...
  shmctl(light_shm_id, IPC_RMID, NULL);
  shmctl(heavy_shm_id, IPC_RMID, NULL);
...
void setup_shm(void) {
  ...
  memset(virgin_tmp_crash, 255, MAP_SIZE);

  light_shm_id = shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | 0600);

  if (light_shm_id < 0) PFATAL("shmget() failed");

  atexit(remove_shm);

  shm_str = alloc_printf("%d", light_shm_id);

  setenv(LIGHT_SHM_ENV_VAR, shm_str, 1);

  ck_free(shm_str);

  trace_bits = shmat(light_shm_id, NULL, 0);

  if (!trace_bits) PFATAL("shmat() failed");

  /* weizz map */
  heavy_shm_id = shmget(IPC_PRIVATE, sizeof(struct cmp_map), IPC_CREAT | IPC_EXCL | 0600);

  if (heavy_shm_id < 0) PFATAL("shmget() failed");

  shm_str = alloc_printf("%d", heavy_shm_id);
  setenv(HEAVY_SHM_ENV_VAR, shm_str, 1);

  cmp_map = shmat(heavy_shm_id, NULL, 0);

  if (!cmp_map) PFATAL("shmat() failed");

  ck_free(shm_str);

  /* weizz map */
  patches_shm_id = shmget(IPC_PRIVATE, WMAP_WIDTH, IPC_CREAT | IPC_EXCL | 0600);

  if (patches_shm_id < 0) PFATAL("shmget() failed");

  shm_str = alloc_printf("%d", patches_shm_id);
  setenv(PATCH_SHM_ENV_VAR, shm_str, 1);

  cmp_patch_map = shmat(patches_shm_id, NULL, 0);

  if (!cmp_patch_map) PFATAL("shmat() failed");
...
void setup_post(void) {
  ...
  void *dh;
  u8 *  fn   = getenv("WEIZZ_POST_LIBRARY");
  u32   tlen = 6;
  ...
  post_handler = dlsym(dh, "weizz_postprocess");
  if (!post_handler) FATAL("Symbol 'weizz_postprocess' not found.");
...
void read_testcases(void) {
  ...
  if (nl_cnt < 0) {
    ...
    if (errno == ENOENT || errno == ENOTDIR)
      ...
      SAYF("\n" cLRD "[-] " cRST
           "The input directory does not seem to be "
  ...
  for (i = 0; i < nl_cnt; i++) {
    ...
    ++initial_testcases_num;

    u8 *tags_fname =
        alloc_printf("%s/tags/%s", out_dir, strrchr(queue_top->fname, '/') + 1);

    if (access(tags_fname, F_OK)) {
    ...
    queue_top->tags_fname = tags_fname;
  ...
  if (!queued_paths) {
    ...
    SAYF("\n" cLRD "[-] " cRST
         "Looks like there are no valid test cases in "
...
void check_binary(u8 *fname) {
  ...
  if (getenv("WEIZZ_SKIP_BIN_CHECK")) return;
  ...
  if (f_data[0] == '#' && f_data[1] == '!') {
    ...
    SAYF("\n" cLRD "[-] " cRST
         "Oops, the target binary looks like a shell "
  ...
  if (!qemu_mode && !memmem(f_data, f_len, LIGHT_SHM_ENV_VAR, strlen(LIGHT_SHM_ENV_VAR) + 1)) {
    ...
    SAYF(
        ...
        "how to\n"
        "    instrument binaries, please see README.\n\n"
  ...
  if (qemu_mode && memmem(f_data, f_len, LIGHT_SHM_ENV_VAR, strlen(LIGHT_SHM_ENV_VAR) + 1)) {
    ...
    SAYF("\n" cLRD "[-] " cRST
         "This program appears to be instrumented with weizz-clang.\n"
         "    This is probably not what you want - this setup will be\n"
         "    slow and offer no practical benefits.\n");

    FATAL("Instrumentation found in QEMU mode");
...
void check_if_tty(void) {
  ...
  if (getenv("WEIZZ_NO_UI")) {

    OKF("Disabling the UI because WEIZZ_NO_UI is set.");
    not_on_tty = 1;
    return;
...
void bind_to_free_cpu(void) {
  ...
  if (getenv("WEIZZ_NO_AFFINITY")) {

    WARNF("Not binding to a CPU core (WEIZZ_NO_AFFINITY set).");
    return;
  ...
  if (i == cpu_core_count) {
    ...
    SAYF("\n" cLRD "[-] " cRST
         ...
         "you are\n"
         "    absolutely sure, you can set WEIZZ_NO_AFFINITY and try again.\n",
         cpu_core_count);
...
static void maybe_delete_out_dir(void) {
  ...
  if (flock(out_dir_fd, LOCK_EX | LOCK_NB) && errno == EWOULDBLOCK) {
    ...
    SAYF("\n" cLRD "[-] " cRST
         "Looks like the job output directory is being "
  ...
  if (f) {
    ...
    if (!in_place_resume && last_update - start_time > OUTPUT_GRACE * 60) {
      ...
      SAYF("\n" cLRD "[-] " cRST
           "The job output directory already exists and "
  ...
  if (!in_place_resume) {
    ...
    fn = alloc_printf("%s/tags", out_dir);
    ...
    fn = alloc_printf("%s/patch_map", out_dir);
    if (unlink(fn) && errno != ENOENT) goto dir_cleanup_failed;
    ck_free(fn);
...
dir_cleanup_failed:
  ...
  SAYF("\n" cLRD "[-] " cRST
       "Whoops, the fuzzer tried to reuse your output "
...
void setup_dirs_fds(void) {
  ...
  /* Tags directory for any starting & discovered paths. */

  tmp = alloc_printf("%s/tags", out_dir);
  if (in_place_resume) {
...
void check_crash_handling(void) {
  ...
  if (!getenv("WEIZZ_I_DONT_CARE_ABOUT_MISSING_CRASHES"))
...
/* Check CPU governor. Backport from afl++ */
...
void check_cpu_governor(void) {
  ...
  if (getenv("WEIZZ_SKIP_CPUFREQ")) return;
  ...
  while (q) {
    ...
    if (!strncmp(rsl, CASE_PREFIX, 3) &&
      ...
      if (use_name)
        ...
        use_name = rsl;
      nfn = alloc_printf("%s/queue/id:%06u,time:%llu,orig:%s", out_dir, id,
                         start_time, use_name);
