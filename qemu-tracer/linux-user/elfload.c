...
/* GDB base address for PIE.
   The binary will be mapped under the libraries and stack, be aware */
//#define PIE_BASE_ADDR 0x555555554000

abi_ulong weizz_entry_point,
          weizz_start_code,
          weizz_end_code,
                           ...
                           char bprm_buf[BPRM_BUF_SIZE])
{
    ...
    if (ehdr->e_type == ET_DYN) {
        ...
        if (pinterp_name != NULL) {
            /* This is the main executable.  To avoid a wring brk
               value it is mapped to a fixed address  */
            probe_guest_base(image_name, PIE_BASE_ADDR, PIE_BASE_ADDR + hiaddr);
            load_addr = PIE_BASE_ADDR;
        } else {
    ...
    if (!weizz_main_elf_start) weizz_main_elf_start = load_addr;
    if (!weizz_entry_point) {
      if (getenv("WEIZZ_ENTRYPOINT") != NULL)
        weizz_entry_point = strtoul(getenv("WEIZZ_ENTRYPOINT"), NULL, 16);
      else
        weizz_entry_point = info->entry;
    }

    int set_main_elf_end = 0;
    if (!weizz_main_elf_end) set_main_elf_end = 1;
    ...
    for (i = 0; i < ehdr->e_phnum; i++) {
        ...
        if (eppnt->p_type == PT_LOAD) {
            ...
            if (set_main_elf_end)
              weizz_main_elf_end = MAX(weizz_main_elf_end, vaddr_ef);
            
            /* Find the full program boundaries.  */
            if (elf_prot & PROT_EXEC) {
                if (vaddr < info->start_code) {
                    info->start_code = vaddr;
                    if (!weizz_start_code) weizz_start_code = vaddr;
                }
                if (vaddr_ef > info->end_code) {
                    info->end_code = vaddr_ef;
                    if (!weizz_end_code) weizz_end_code = vaddr_ef;
                }
            }
