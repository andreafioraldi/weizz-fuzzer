...
static void weizz_log_br(void* cur_loc)
{
    uint64_t afl_index = ((abi_ulong)cur_loc ^ prev_loc ^ shadow_hash) & (MAP_SIZE -1);
...
void weizz_gen_trace(target_ulong cur_loc)
{
    if (!ADDR_IS_IN_BOUND(cur_loc))
        return;
