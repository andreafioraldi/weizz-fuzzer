...
//TODO backport from afl++
#define WEIZZ_QEMU_TARGET_i386_SNIPPET                                        \
  if (is_persistent) {                                                        \
                                                                              \
    if (s->pc == weizz_persistent_addr) {                                       \
                                                                              \
      if (weizz_persistent_ret_addr == 0) {                                     \
                                                                              \
      ...
      TCGv_ptr paddr = tcg_const_ptr(weizz_persistent_addr);                    \
      tcg_gen_st_tl(paddr, cpu_regs[R_ESP], 0);                               \
      tcg_gen_weizz_callN(weizz_persistent_loop, NULL, 0, NULL);  \
                                                                              \
    } else if (weizz_persistent_ret_addr && \
               s->pc == weizz_persistent_ret_addr) { \
                                                                              \
      gen_jmp_im(s, weizz_persistent_addr);                                     \
      gen_eob(s);                                                             \
                                                                              \
    }                                                                         \
                                                                              \
...
static inline void weizz_log_cmp(
  ...
  //if (weizz_target_cmp && weizz_target_cmp != (target_ulong)addr)
    //  return;

  uintptr_t k = pc ^ shadow_hash;
  k &= WMAP_WIDTH - 1; 
  
  heavy_map->headers[k].id = pc;
...
__thread uintptr_t shadow_hash;
...
static inline int is_valid_addr(target_ulong addr)
{
    ...
    if (l < STR_LOG_SIZE) {
        ...
        page = (addr +STR_LOG_SIZE) & TARGET_PAGE_MASK;
...
__thread target_ulong cmps_last_rcx;
__thread target_ulong cmps_last_pc;

static void weizz_log_cmps(target_ulong rcx, target_ulong rdi, target_ulong rsi, target_ulong pc) {

    //if (is_valid_addr(rdi) && is_valid_addr(rsi)) {
        if (cmps_last_pc == pc) {
          if(cmps_last_rcx > rcx) {
            cmps_last_rcx = rcx;
            return;
          }
        }
        ...
        cmps_last_rcx = rcx;
        cmps_last_pc - pc;
        weizz_log_rtn(pc, rdi, rsi, STR_LOG_SIZE-1);
    //}
}

static void weizz_gen_log_cmps(target_ulong pc) {

  if (ADDR_IS_IN_BOUND(pc)) {
  
        pc = (pc >> 4) ^ (pc << 8);
        pc &= WMAP_WIDTH - 1;
        
        TCGTemp *args[4] = { tcgv_i64_temp( cpu_regs[R_ECX] ),
                             tcgv_i64_temp( cpu_regs[R_EDI] ),
                             tcgv_i64_temp( cpu_regs[R_ESI] ),
                             tcgv_ptr_temp( tcg_const_tl(pc) ) };
        tcg_gen_weizz_callN(weizz_log_cmps, NULL, 4, args);
    }

}

static void heavy_shadow_push(target_ulong rdi, target_ulong rsi, target_ulong pc, target_ulong jmp_to)
{
    if (is_valid_addr(rdi) && is_valid_addr(rsi))
      weizz_log_rtn(pc, rdi, rsi, STR_LOG_SIZE-1);

    if (ctx_sensitive && ADDR_IS_IN_BOUND(jmp_to))
      shadow_hash ^= pc;
}

static void light_shadow_push(target_ulong pc, target_ulong jmp_to)
{
    if (!ADDR_IS_IN_BOUND(jmp_to))
        return;

    shadow_hash ^= pc;
}

static void shadow_pop(target_ulong jmp_to)
{
    if (!ADDR_IS_IN_BOUND(jmp_to))
        return;
    
    target_ulong cur_loc = jmp_to -5;
    
    jmp_to = (jmp_to >> 4) ^ (jmp_to << 8);
    jmp_to &= WMAP_WIDTH - 1;
...
static void gen_shadow_push(target_ulong pc, TCGv_i64 jmp_to)
{
    if (ADDR_IS_IN_BOUND(pc)) {
        ...
        if (is_heavy) {
            TCGTemp *args[4] = { tcgv_i64_temp( cpu_regs[R_EDI] ),
                                 tcgv_i64_temp( cpu_regs[R_ESI] ),
                                 tcgv_ptr_temp( tcg_const_tl(pc) ),
                                 tcgv_i64_temp(jmp_to) };
            tcg_gen_weizz_callN(heavy_shadow_push, NULL, 4, args);
        } else {
            TCGTemp *args[2] = { tcgv_ptr_temp( tcg_const_tl(pc) ),
                                 tcgv_i64_temp(jmp_to) };
            tcg_gen_weizz_callN(light_shadow_push, NULL, 2, args);
        }
    }
}

static void imm_shadow_push(target_ulong pc, target_ulong jmp_to)
{
    if (ADDR_IS_IN_BOUND(pc)) {
        ...
        if (is_heavy) {
            TCGTemp *args[4] = { tcgv_i64_temp( cpu_regs[R_EDI] ),
                                 tcgv_i64_temp( cpu_regs[R_ESI] ),
                                 tcgv_ptr_temp( tcg_const_tl(pc) ),
                                 tcgv_i64_temp( tcg_const_tl(jmp_to) ) };
            tcg_gen_weizz_callN(heavy_shadow_push, NULL, 4, args);
        } else {
            TCGTemp *args[2] = { tcgv_ptr_temp( tcg_const_tl(pc) ),
                                 tcgv_i64_temp( tcg_const_tl(jmp_to) ) };
...
static void gen_shadow_pop(target_ulong pc, TCGv_i64 jmp_to)
{
    TCGTemp *args[1] = { tcgv_i64_temp(jmp_to) };
    tcg_gen_weizz_callN(shadow_pop, NULL, 1, args);
}

/* Cmp patching with a bit of magic */

/* v0 = v0 | (sign extend cmp_patch_map[pc])
   r_op = v1 | (sign extend cmp_patch_map[pc])
   v0 = v0 - r_op */
#define weizz_gen_sub_with_patch_mask(pc, v0, v1) \
do { \
    target_ulong pc_hash = ((pc) >> 4) ^ ((pc) << 8); \
    pc_hash &= WMAP_WIDTH - 1; \
     \
    TCGv r_op = tcg_temp_new(); /* We don't want side effects on v1 */ \
    TCGv_ptr mask_byte_ptr = tcg_const_ptr(cmp_patch_map +pc_hash); \
    TCGv cmp_mask = tcg_temp_new(); \
    tcg_gen_ld8s_tl(cmp_mask, mask_byte_ptr, 0); \
     \
    tcg_gen_or_tl(v0, v0, cmp_mask); \
    tcg_gen_or_tl(r_op, v1, cmp_mask); \
     \
    tcg_gen_sub_tl(v0, v0, r_op); \
    \
    tcg_temp_free(r_op); \
    tcg_temp_free(cmp_mask); \
} while(0)

/* l_op = v0 | (sign extend cmp_patch_map[pc])
   r_op = v1 | (sign extend cmp_patch_map[pc])
   ret = l_op - r_op */
#define weizz_gen_cmp_with_patch_mask(pc, ret, v0, v1) \
do { \
    target_ulong pc_hash = ((pc) >> 4) ^ ((pc) << 8); \
    pc_hash &= WMAP_WIDTH - 1; \
     \
    TCGv l_op = tcg_temp_new(); /* We don't want side effects on v0 */ \
    TCGv r_op = tcg_temp_new(); /* We don't want side effects on v1 */ \
    TCGv_ptr mask_byte_ptr = tcg_const_ptr(cmp_patch_map +pc_hash); \
    TCGv cmp_mask = tcg_temp_new(); \
    tcg_gen_ld8s_tl(cmp_mask, mask_byte_ptr, 0); \
     \
    tcg_gen_or_tl(l_op, v0, cmp_mask); \
    tcg_gen_or_tl(r_op, v1, cmp_mask); \
     \
    tcg_gen_sub_tl(ret, l_op, r_op); \
    \
    tcg_temp_free(l_op); \
    tcg_temp_free(r_op); \
    tcg_temp_free(cmp_mask); \
...
static void weizz_gen_log_cmp(target_ulong pc, TCGv_i64 arg1, TCGv_i64 arg2)
{
    if (ADDR_IS_IN_BOUND(pc)) {
        ...
        TCGTemp *args[3] = { tcgv_ptr_temp( tcg_const_tl(pc) ),
                             tcgv_i64_temp(arg1),
                             tcgv_i64_temp(arg2) };
        tcg_gen_weizz_callN(weizz_log_cmp, NULL, 3, args);
...
static void gen_op(DisasContext *s1, int op, TCGMemOp ot, int d)
{
    ...
    case OP_SUBL:
        if (s1->prefix & PREFIX_LOCK) {
            ...
            if (is_heavy) weizz_gen_log_cmp(s1->pc, s1->cc_srcT, s1->T1);

            if (enable_cmp_patching && ADDR_IS_IN_BOUND(s1->pc))
              weizz_gen_cmp_with_patch_mask(s1->pc, s1->T0, s1->cc_srcT, s1->T1);
            else
              tcg_gen_sub_tl(s1->T0, s1->cc_srcT, s1->T1);
        } else {
            tcg_gen_mov_tl(s1->cc_srcT, s1->T0);

            if (is_heavy) weizz_gen_log_cmp(s1->pc, s1->T0, s1->T1);

            if (enable_cmp_patching && ADDR_IS_IN_BOUND(s1->pc))
              weizz_gen_sub_with_patch_mask(s1->pc, s1->T0, s1->T1);
            else
              tcg_gen_sub_tl(s1->T0, s1->T0, s1->T1);
    ...
    case OP_CMPL:
        ...
        if (enable_cmp_patching && ADDR_IS_IN_BOUND(s1->pc))
          weizz_gen_cmp_with_patch_mask(s1->pc, cpu_cc_dst, s1->T0, s1->T1);
        else
...
static target_ulong disas_insn(DisasContext *s, CPUState *cpu)
{
    ...
    WEIZZ_QEMU_TARGET_i386_SNIPPET
 ...
 reswitch:
    ...
    case 0xff: /* GRP5 */
        ...
        case 2: /* call Ev */
            ...
            if (is_heavy || ctx_sensitive)
              gen_shadow_push(next_eip, s->T0);
